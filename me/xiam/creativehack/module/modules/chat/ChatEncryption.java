...
public class ChatEncryption extends Module
{
    ...
    public ChatEncryption() {
        ...
        final TextComponentString chatComponent;
        this.receiveListener = new Listener<PacketEvent.Receive>(event -> {
            if (event.getPacket() instanceof SPacketChat) {
                s3 = ((SPacketChat)event.getPacket()).getChatComponent().getUnformattedText();
                ...
                switch (this.mode.getValue()) {
                    ...
                    case SHIFT: {
                        if (!s3.endsWith("\ud83d\ude48")) {
                            ...
                            s5.chars().forEachOrdered(value -> builder2.append((char)(value + (ChatAllowedCharacters.isAllowedCharacter((char)value) ? (-this.key.getValue()) : 0))));
                            break;
                        }
                        break;
                    }
                ...
                new TextComponentString("§b" + username + '§' + "r: " + builder2.toString());
                sPacketChat.chatComponent = (ITextComponent)chatComponent;
            }
