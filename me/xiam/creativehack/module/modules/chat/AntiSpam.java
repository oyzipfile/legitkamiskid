...
public class AntiSpam extends Module
{
    ...
    private Setting<Boolean> filterDMs;
    ...
    public AntiSpam() {
        ...
        this.filterDMs = this.register(Settings.booleanBuilder("Filter DMs").withValue(false).withVisibility(v -> this.p.getValue().equals(Page.TWO)).build());
    ...
    private boolean isSpam(String message) {
        final ChatTimestamp chatTimestamp = KamiMod.MODULE_MANAGER.getModuleT(ChatTimestamp.class);
        ...
        return (this.filterOwn.getValue() || !this.findPatterns(OWN_MESSAGE, message, false)) && !MessageDetectionHelper.isDirect(this.filterDMs.getValue(), message) && !MessageDetectionHelper.isDirectOther(this.filterDMs.getValue(), message) && this.detectSpam(this.removeUsername(message));
    }
        ...
        if (this.duplicates.getValue()) {
            ...
            if (isDuplicate) {
                if (this.showBlocked.getValue().equals(ShowBlocked.CHAT)) {
                    MessageSendHelper.sendChatMessage(this.getChatName() + "Duplicate: " + message);
                }
                else if (this.showBlocked.getValue().equals(ShowBlocked.LOG_FILE)) {
                    KamiMod.log.info(this.getChatName() + "Duplicate: " + message);
                }
            }
        }
        return false;
    ...
    private void sendResult(final String name, final String message) {
        if (this.showBlocked.getValue().equals(ShowBlocked.CHAT)) {
            MessageSendHelper.sendChatMessage(this.getChatName() + name + ": " + message);
        }
        else if (this.showBlocked.getValue().equals(ShowBlocked.LOG_FILE)) {
            KamiMod.log.info(this.getChatName() + name + ": " + message);
        }
    }
    ...
    private static class FilterPatterns
    {
        ...
        static {
            ...
            I_JUST_THANKS_TO = new String[] { "i just.*thanks to", "i just.*using" };
