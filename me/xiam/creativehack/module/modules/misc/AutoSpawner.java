...
public class AutoSpawner extends Module
{
    ...
    private boolean isCreativeAndMoreThenOne(final int stack) {
        return AutoSpawner.mc.player.inventory.getStackInSlot(stack).stackSize >= 1 && AutoSpawner.mc.playerController.currentGameType.equals((Object)GameType.CREATIVE);
    }
            ...
            if (stack != ItemStack.EMPTY) {
                if (this.entityMode.getValue().equals(EntityMode.WITHER)) {
                    if (stack.getItem() == Items.SKULL && stack.getItemDamage() == 1) {
                        if (AutoSpawner.mc.player.inventory.getStackInSlot(i).stackSize >= 3 || this.isCreativeAndMoreThenOne(i)) {
                            this.headSlot = i;
                        }
                        continue;
                    }
                        ...
                        if (block instanceof BlockSoulSand && (AutoSpawner.mc.player.inventory.getStackInSlot(i).stackSize >= 4 || this.isCreativeAndMoreThenOne(i))) {
                            this.bodySlot = i;
                        }
                    }
                }
                if (this.entityMode.getValue().equals(EntityMode.IRON)) {
                    ...
                    if (block == Blocks.IRON_BLOCK && (AutoSpawner.mc.player.inventory.getStackInSlot(i).stackSize >= 4 || this.isCreativeAndMoreThenOne(i))) {
                        this.bodySlot = i;
                    }
                }
                if (this.entityMode.getValue().equals(EntityMode.SNOW)) {
                    if (stack.getItem() instanceof ItemBlock) {
                        ...
                        if (block == Blocks.SNOW && (AutoSpawner.mc.player.inventory.getStackInSlot(i).stackSize >= 2 || this.isCreativeAndMoreThenOne(i))) {
                            this.bodySlot = i;
                        }
                    }
                }
    ...
    public void onUpdate() {
        ...
        if (this.buildStage == 1) {
            ...
            if (!this.checkBlocksInHotbar()) {
                if (!this.party.getValue()) {
                    if (this.debug.getValue()) {
                        MessageSendHelper.sendChatMessage(this.getChatName() + ChatFormatting.RED.toString() + "Blocks missing for: " + ChatFormatting.RESET.toString() + this.entityMode.getValue().toString() + ChatFormatting.RED.toString() + ", disabling.");
                    }
                    this.disable();
                }
                return;
            ...
            final List<BlockPos> blockPosList = crystalAura.getSphere(AutoSpawner.mc.player.getPosition().down(), this.placeRange.getValue(), this.placeRange.getValue().intValue(), false, true, 0);
            ...
            if (noPositionInArea) {
                if (this.useMode.getValue().equals(UseMode.SINGLE)) {
                    if (this.debug.getValue()) {
                        MessageSendHelper.sendChatMessage(this.getChatName() + ChatFormatting.RED.toString() + "Position not valid, disabling.");
                    }
                    this.disable();
                }
                return;
