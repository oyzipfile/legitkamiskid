...
public class ElytraFlight extends Module
{
    ...
    public ElytraFlight() {
        ...
        this.timerControl = this.register(Settings.booleanBuilder("Takeoff Timer").withValue(false).withVisibility(v -> this.easyTakeOffControl.getValue() && this.mode.getValue().equals(ElytraFlightMode.CONTROL)).build());
        ...
        this.fallSpeedControl = this.register(Settings.floatBuilder("Fall Speed C").withValue(0.001f).withMaximum(0.3f).withMinimum(0.0f).withVisibility(v -> this.mode.getValue().equals(ElytraFlightMode.CONTROL)).build());
        ...
        this.playerTravelListener = new Listener<PlayerTravelEvent>(event -> {
            if (this.mode.getValue().equals(ElytraFlightMode.CONTROL) && ElytraFlight.mc.player != null) {
                if (!ElytraFlight.mc.player.isElytraFlying()) {
                    ...
                    if (moveUp || moveForward || moveBackward || moveLeft || moveRight || KamiMod.MODULE_MANAGER.isModuleEnabled(AutoWalk.class)) {
    ...
    public void onUpdate() {
        ...
        if (this.enabledMode != this.mode.getValue() && !this.hasDoneWarning) {
            MessageSendHelper.sendErrorMessage("&l&cWARNING:&r Changing the mode while you're flying is not recommended. If you weren't flying you can ignore this message.");
    ...
    private void takeOff() {
        ...
        if (ElytraFlight.mc.player.isElytraFlying()) {
            ...
            MessageSendHelper.sendChatMessage(this.getChatName() + "Disabled takeoff!");
        }
    }
